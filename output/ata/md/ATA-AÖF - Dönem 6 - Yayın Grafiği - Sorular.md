...
1. **Aşağıdakilerden hangisi gazetelerin özellikleri arasında yer almaz?**
    ...
    ***
1. **Aşağıdakilerden hangisi Sanayi Devrimi ile gelişen matbaa makineleri arasında yer almaz?**
...
1. **Aşağıdakilerden hangisi elektronik yayıncılığın kazanımları arasında yer almaz?**
    ...
    ***
1. **Aşağıdakilerden hangisi çağlar boyunca bilgi aktarım araçlarının başında yer almıştır?**
    - A-) Dergi
    - B-) Gazete
    - C-) El ilanı
    - **Cevap D-) Kitap**
...
1. **Aşağıdakilerden hangisi grafik tasarımcı kelimesini oluşturan köklerden biri değildir?**
    ...
    - E-) Tasarlayan
    ***
1. **Aşağıdakilerden hangisi hem sanat hem de tasarım üretimi için ortak bir konudur?**
...
1. **Yayın grafiği tasarımının ele aldığı ana konulardan bazıları aşağıdakilerden hangisinde doğru olarak belirtilmiştir?**
    ...
    ***
1. **Bulunan bir fikrin eski bir tasarımda kullanılmadığından emin olmak isteyen kişi aşağıdakilerden hangisini yapmalıdır?**
    ...
    ***
1. **Beyin Fırtınası Tekniği aşağıdaki amaçlardan hangisi için kullanılmaktadır?**
    ...
    ***
1. **Aşağıdakilerden hangisi tasarımın sanattan farklı bir hedefidir?**
    - A-) Güzel görünmek
    - B-) Yenilikçi olmak
    - C-) İlgi çekici olmak
    - D-) Farklı olmak
...
1. **Aşağıdakilerden hangisi yayın grafiğinde tasarım ilkelerinden vurgu ilkesini tanımlar?**
    ...
    ***
1. **Aşağıdakilerden hangisi formu tanımlar?**
    ...
    ***
1. **Aşağıdakilerin hangisinde sıcak ve soğuk renkler sıralanmıştır?**
    ...
    ***
1. **Aşağıdakilerden hangisi ritmi tanımlamaktadır?**
...
1. **Aşağıdakilerden hangisi yayın grafiklerinde tasarımcının duyarlılık geliştirmesi açısından önemli olup, belli standartları gerektiren bir argümandır?**
    ...
    - E-) Vurgu
    ***
1. **Aşağıdakilerden hangisi bir yayın grafiği tasarımı değildir?**
    - A-) Dergi tasarımı
    - B-) Kitap tasarımı
    - C-) Broşür tasarımı
    - D-) Gazete tasarımı
...
1. **4+0 CMYK baskı nedir?**
    ...
    - E-) Gofre Baskı
    ***
1. **Gutenberg Diyagramı'na göre okuma yer çekimi hangi yöne hareket etmektedir?**
    ...
    ***
1. **Aşağıdaki programlardan hangisi piksel tabanlıdır?**
...
1. **Aşağıdakilerden hangisi tasarıma uygun şekilde büyültülüp küçültülmesine rağmen çözünürlüğü bozulmayan grafik şekillerdir?**
    ...
    - E-) RGB
    ***
1. **Damlalık aracı aşağıdakilerden hangisini yapmamızı sağlamaktadır?**
    ...
    ***
1. **Masaüstü yayıncılık için daha çok araç bulunduran yazılım hangisidir?**
    ...
    - E-) Alanları seçmek
    ***
1. **Yazılımdaki araçların kısayollarını değiştirmek için hangi işlemleri yapmamız gerekmektedir?**
    - A-) File, Package
    - B-) Window, Toolbar
    - C-) File, Export
    ...
    - E-) Window, Workspace
    ***
1. **Yazılım kullanırken sayfa alanını aşağı yukarı hareket ettiren ''El Aracını'', klavyemizin hangi tuşuyla aktif hâle getirebiliriz?**
    - A-) Ctrl
    - B-) Enter
    - C-) Delete
    - D-) Alt
...
1. **Aşağıdakilerden hangisi sayfa tasarımında bir kılavuzun ana bileşenlerinden biri değildir?**
    ...
    - E-) Modül
    ***
1. **Aşağıdakilerden hangisi sayfa tasarımı için yanlıştır?**
    ...
    ***
1. **Aşağıdakilerden hangisi sayfa tasarımı kılavuz ızgara sistemlerinden biri değildir?**
    - A-) Modüler ızgaralar
...
1. **Aşağıda yayın grafiğinde sayfa tasarımı ile ilgili verilen bilgilerden hangisi yanlıştır?**
    ...
    ***
1. **Aşağıdakilerden hangisi süreli yayın grafiği tasarımı süreçlerinden biri değildir?**
...
1. **Aşağıdakilerden hangisinde Gazete ve dergi arasındaki farklar doğru bir şekilde verilmiştir?**
    ...
    ***
1. **Aşağıdakilerden hangisinde süreli yayın grafiği süreçleri doğru bir şekilde verilmiştir?**
    ...
    ***
1. **Aşağıdakilerden hangisi süreli yayın grafiği baskı ve çoğaltım aşamasındaki süreçlerden biri değildir?**
    - A-) Forma düzeni
    - B-) Renk ayrımı
    ...
    - E-) Baskı modeli
    ***
1. **Aşağıdakilerden hangisi ile sayfa tasarımında grid (ızgara) düzeniyle ayarlamalar yapılamaz?**
    - A-) Ölçeklendirme
    - B-) Marj boşlukları
    - C-) Modül sistemi
    ...
    ***
1. **Aşağıdakilerden hangisinde süreli yayın grafiği tasarımında kullanılan bir kılavuzun ana bileşenlerinden olan işaretçiler için yanlıştır?**
...
1. **İki harf arasındaki boşluğun uyumlanması işlemi aşağıdakilerden hangisidir?**
    ...
    - E-) Hyphenating
    ***
1. **İki yana yaslanmış metinlerde ortaya çıkan ve kelimeler arasında oluşan fazla boşlukların satırlar boyunca aşağı doğru oluşturduğu beyaz boşluğa ne ad verilir?**
    - A-) Dul
    - B-) Yetim
    - C-) Öksüz
    - D-) Kırıntı
    - **Cevap E-) Nehir**
    ***
1. **Aşağıdakilerken hangisi yazı sınıflarından biri değildir?**
    - A-) Modern
    - **Cevap B-) Post-modern**
    - C-) Tırnaklı
    - D-) Tırnaksız
    ...
    ***
## Unite 7
1. **Fotoğraf makinalarındaki fotoğraflanması hedeflenen alanı tanımlayan çerçeve aşağıdakilerden hangisidir?**
    - A-) Diyafram
    - B-) Deklanşör
    - **Cevap C-) Vizör**
    - D-) Örtücü
...
1. **Aşağıdakilerden hangisinde biçim-içerik ilişkisi kurulmamıştır?**
    ...
    - E-) Çöl - sıcak
    ***
1. **Aşağıdakilerden hangisi bir fotoğrafın değiştirilemeyen özelliklerinden biridir?**
    - A-) Netlik
    - **Cevap B-) Çözünürlük**
    - C-) Ton
    - D-) Renk
...
1. **Gestalt kuramı aşağıdakilerden hangi alanla ilişkilidir?**
    ...
    - E-) Biçimlendirme
    ***
1. I. İllustrasyon<br />II. Fotoğraf<br />III. Şekil<br />**Yukarıdakilerden hangisi ya da hangileri yayın grafiğinin görsel unsurları arasında yer alır?**
    - A-) Yalnız I
    - B-) Yalnız III
    - C-) I ve II
    - D-) II ve III
    - **Cevap E-) I, II ve III**
    ***
1. **Aşağıdakilerden hangisi bir kompozisyon kuralı değildir?**
    - A-) Üçler
    - B-) Boşluklar
    - C-) Yığma
    - **Cevap D-) Birler**
    - E-) Tekler
    ***
1. **Aşağıdakilerden hangisi yayın grafiğinde kullanılan görsel unsurlardan biridir?**
    - A-) Folklör
    ...
    - E-) Film
    ***
1. **Aşağıdakilerden hangisi fotoğrafın işlevleri arasında yer almaz?**
    - A-) Duygulandırmak
    - B-) Belgelemek
    - C-) Bilgilendirmek
    ...
    - E-) Düşündürmek
    ***
1. **Bir kompozisyonda aşağıdaki özelliklerden hangisi aranmaz?**
    - A-) Oran-orantı
    - B-) Bütünlük
    - C-) Uyum
    - D-) Denge
    - **Cevap E-) Armoni**
    ***
1. **Aşağıdakilerden hangisi nötr renklerden biri değildir?**
    - A-) Kahverengi
    - B-) Beyaz
    - **Cevap C-) Mürdüm**
    - D-) Siyah
...
1. **Aşağıdakilerden hangisi etkili bir sloganın yaratılmasındaki kriterlerden biri değildir?**
    ...
    - **Cevap E-) Uzun olması**
    ***
1. **Aşağıdakilerden hangisi bir amblemin başarılı olmasındaki etkenlerden biri değildir?**
    - A-) Özgün
    - B-) Estetik
    - **Cevap C-) Öznel**
    - D-) Dikkat çekici
    - E-) Firmayı tanıtıcı
    ***
1. Tasarım gerçekleştirilmeden önce kılavuzluk yapması için sayfanın yatay ve dikey çizgilerle bölünmesidir.<br />**Bu tanım aşağıdakilerden hangisine aittir?**
    - A-) Denge
    - B-) Perspektif
    - C-) Vurgu
    - **Cevap D-) Izgara**
    - E-) Hiyerarşi
    ***
1. “Reklamın konusu ve mesajı hakkında hedef kitleye ayrıntılı bilgilendirme sunmaktadır.”<br />**Bu tanım aşağıdakilerden hangisine aittir?**
    - A-) Başlık
    - B-) Alt Başlık
    - **Cevap C-) Metin**
    - D-) Slogan
    - E-) Fotoğraf
    ***
1. **"Markayı ifade eden harf, kelime veya rakamlardan meydana gelmektedir." şeklinde tanımlanan kavram aşağıdakilerden hangisidir?**
    - **Cevap A-) Logo**
    - B-) Amblem
    - C-) Slogan
    - D-) Metin
...
1. **Aşağıdakilerden hangisi reklam tasarımındaki kompozisyon ve sayfa düzeni ilkelerinden biri değildir?**
    ...
    ***
## Unite 9
1. **Yayında kullanılan özel terimlerin, kişilerin ya da kavramların hangi sayfalarda göründüklerini gösteren liste aşağıdakilerden hangisidir?**
    - A-) Sözlük
    - **Cevap B-) Dizin**
    - C-) İçindekiler
    - D-) Referanslar
    - E-) Künye
    ***
1. **Aşağıdakilerden hangisi çok sayfalı yayınlarda başlıkların işlevlerinden biri değildir?**
...
1. **Aşağıdakilerden hangisi çok sayfalı yayını oluşturan fiziksel kısımlardan biri değildir?**
    ...
    - **Cevap E-) Su yolu**
    ***
1. **Künye sayfasında aşağıdakilerden hangisi yer almaz?**
...
1. **Yazarın yayını birine adamak ya da hediye etmek için yazmış olduğu kısa notunu içeren sayfa aşağıdakilerden hangisidir?**
    ...
    - E-) Giriş
    ***
1. **InDesign programında diğer tüm sayfalar için şablonlar oluşturma, sayfa numarası atama ve değişiklikler yapma olanağı sağlayan özellik aşağıdakilerden hangisidir?**
    ...
    ***
## Unite 10
1. **Aşağıdakilerden hangisi geleneksel k****â****ğıt standartlarından biri değildir?**
    - A-) Emperor/İmparator
    ...
    - E-) Royal/Asil
    ***
1. **Otokopi türü kâğıtlar arasında aşağıdakilerden hangisi yer almaz?**
    - A-) İrsaliye kağıdı
    - B-) Makbuz kağıdı
    - C-) Fatura kağıdı
    - D-) Sipariş kağıdı
    - **Cevap E-) Kuşe kağıdı**
    ***
1. **Aşağıdakilerden hangisi baskı öncesi hazırlıklarda yer alan ciltleme hedefi kazanımlarından biri değildir?**
    - A-) Maliyetten kazanç
    - B-) Estetik görünüm
...
1. **Aşağıdakilerden hangisi baskı öncesi hazırlık aşamalarından biri değildir?**
    ...
    - E-) Ciltleme seçimi
    ***
1. **Aşağıdakilerden hangisi kâğıt standartları arasında yer almaz?**
    - A-) ISO
    - B-) DIN
    - **Cevap C-) ÇİN**
    - D-) JAPON
    - E-) KANADA
    ***
1. **Aşağıdaki ölçülerden hangisi ISO kağıt standartlarından biri olarak yer almaktadır?**
...
1. **Aşağıdakilerden hangisi yayın grafiği basımında 1970’li yıllara kadar 500 yılı aşkın bir süreyle kullanılan baskı türüdür?**
    ...
    - E-) Flesko Baskı
    ***
1. **Aşağıdakilerden hangisinde baskı öncesi aşamalar doğru bir şekilde sıralanmıştır?**
    ...
    ***
1. **Aşağıdakilerden hangisi tabaka k****â****ğıdın standart ebatlarına göre uyarlanmış bir forma düzeni değildir?**
...
1. **Aşağıdakilerden hangisi düz baskı sistemi ile üretilen matbaacılık türüdür?**
    ...
    - E-) Gofre Baskı
    ***
1. **Web ofset matbaacılık ile ilgili olarak aşağıdakilerden hangisi yanlıştır?**
...
1. **Aşağıdakilerden hangisi çukur baskı sitemi matbaa teknolojisidir?**
    ...
    - E-) Tipo Baskı
    ***
1. **Aşağıdakilerden hangisi baskı öncesi dikkat edilmesi gereken görevlerden biri değildir?**
    ...
    ***
1. **Aşağıdakilerden hangisi baskı aşamasında bilinmesi gereken önemli aşamalardan biri olamaz?**
    - A-) Kağıt özellikleri
    - B-) Renk ve Tonlama
    - C-) Forma Düzeni
    - D-) Tram ve Montaj
...
1. **Aşağıdakilerden hangisi CMYK renk modellerinden biri değildir?**
    ...
    ***
## Unite 12
1. **Dijital ortamda yayınlanacak bir yayın grafiği tasarımı ürününün ekran üzerinde görüntülenebilmesi için hangi renk ve dosya biçimi formatlarında düzenlenmiş olması gerekmektedir?**
    - A-) CMYK – PDF
    - **Cevap B-) RGB - PDF**
    - C-) CMYK – Jpeg
    - D-) RGB – Pantone
    - E-) PDF – Jpeg
    ***
1. **Aşağıdakilerden hangisi Dijital Yayın Grafiği ürünlerinin (e-kitap, e-dergi gibi) okuyucuya sunduğu avantajlardan biri değildir?**
...
1. **Aşağıdakilerden hangisi e-gazetelerin avantajlarından biri değildir?**
    ...
    ***
1. Dijital ortamda sergilenen dergi, kitap benzeri tasarımların sayfa kaydırma efektli (Flip) yüklenmesi, …………………bakımından, tasarımcı ve kullanıcı için cazip hâle gelmektedir.<br />**Cümle boş bırakılan yere aşağıdakilerden hangisi getirilmelidir?**
...
1. **Hangisi tasarım uygulamalarında kullanılan ekran rengidir?**
    ...
    - E-) Pantone
    ***
1. **Bir fotoğraf veya görüntüyü dijital bir ortama yüklerken hangi çözünürlük ayarı genellikle tercih edilmelidir?**
    - A-) 300 dpi.
    - **Cevap B-) 600dpi.**
    - C-) 36 dpi.
    - D-) 72 dpi.
...
1. **Hangisi dijital dergi ve katalog oluşturmada İssuu, Yumpu, Joomag, Calemeo gibi sitelerin avantajlarından biri değildir?**
    ...
    ***
1. **Aşağıdakilerden hangisi e-kitap okumak için kullanılmamaktadır?**
    - A-) Telefon
    - B-) Tablet
    - C-) Bilgisayar
    - **Cevap D-) Televizyon**
...
1. **Aşağıdakilerden hangisi sayfa kenar boşluklarının işlevleri arasında yer alır?**
    ...
    ***
1. **Aşağıdakilerden hangisi modüler ızgara çarpanları (örüntü) arasında yer almaz?**
    - A-) 3x4
    - B-) 3x6
    - C-) 5x8
    - D-) 5x12
    - **Cevap E-) 7x14**
    ***
1. **Aşağıdakilerden hangisi istatistiklerde neredeyse hiç tercih edilmeyen ekran ölçüsüdür?**
...
1. ………………….. web site tasarımı, farklı ekran formatlarında, bilginin kolaylıkla kaydırılıp hizalanması, ortama göre şekillenmesi ve uyum sağlamasıdır. **Boş bırakılan bölgeye aşağıdakilerden hangisi yer alır?**
    ...
    ***
## Unite 14
1. **Aşağıdakilerden hangisi hareketli grafik tasarımında storyboard’un kullanılabileceği alanlardan biri değildir?**
    ...
    - E-) Film jenerikleri
    ***
1. **Aşağıdakilerden hangisi, hareketin yavaşlayarak veya hızlanarak gerçekleştirilmesinin projeye kattığı bir katkıdır?**
    ...
    ***
1. **Aşağıdakilerden hangisi vektör tabanlı görüntü ile üretilmiş bir hareketli grafik görüntünün piksel tabanlı hareketli görüntüye karşı avantajıdır?**
    ...
    ***
1. **Aşağıdakilerden hangisi, canlandırma tasarımında cazibe yaratma yöntemlerinden biri değildir?**
    ...
    ***
1. **Aşağıdakilerden hangisi konu olarak canlandırmada ezilme ve esneme prensibinin uygulanması için uygun değildir?**
    - **Cevap A-) Metal küre**
    - B-) Su dolu balon
    - C-) Plastik top
    - D-) Jelibon şeker
...
1. **Aşağıdakilerden hangisi, doğrudan ve uç kare canlandırma yapımının gerçekleştirilme yöntemine ilişkin olarak doğru değildir?**
    ...
    ***
1. **Aşağıdakilerden hangisi hareketli grafik tasarımında hareketlerin planlanmasının faydalarından biri değildir?**
